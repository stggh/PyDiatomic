...
def xs(wavenumber, dipolemoment, ei, mu, R, VT, wfi, rot):
...
def xs_vs_rot(rot, dipolemoment, ei, mu, R, VT, wfi, wavenumber):
    """ solve CSE of upper coupled states for the given rotational excitation.

        mirrors xs_vs_wav multiprocessing

        spawn each rotation to a separate cpu

    """
    pool = multiprocessing.Pool()
    func = partial(xs, wavenumber, dipolemoment, ei, mu, R, VT, wfi)

    xsp = pool.map(func, rot)
    xsp = np.array(xsp)
    pool.close()
    pool.join()
...
def xs_vs_wav(wavenumber, dipolemoment, ei, rot, mu, R, VT, wfi):
    """ mulitprocessor pool function.
        ...
        spawn each wavelength to a separate cpu

    """
    pool = multiprocessing.Pool()
    func = partial(xs_vs_rot, dipolemoment, ei, rot, mu, R, VT, wfi)
